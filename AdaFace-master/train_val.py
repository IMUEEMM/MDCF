...
class Trainer(LightningModule):
    def __init__(self, **kwargs):
        ...
        # 1. 存储超参数: 这行代码的作用是将 kwargs 存储为 self.hparams，这样就可以在类的任何地方访问超参数，如 self.hparams.arch。
        self.save_hyperparameters()  # sets self.hparams

        # 2. 计算类别数: 该函数 utils.get_num_class(self.hparams) 主要用于计算数据集的类别数。  结果存储在 self.class_num 中，并打印出来，确保类别数正确。
        self.class_num = utils.get_num_class(self.hparams)
        print('classnum: {}'.format(self.class_num))

        # 3. 构建模型: self.hparams.arch 指定了模型的架构，如 ResNet50 或 MobileNet。   net.build_model() 负责构造指定的神经网络。
        self.model = net.build_model(model_name=self.hparams.arch)
        # 4. 构建分类头: build_head() 可能是用于构建 ArcFace、CosFace 等不同类型的分类头部。
        self.head = head.build_head(head_type=self.hparams.head,        # 头部类型
                                     embedding_size=512,                # 设定特征向量维度
                                     class_num=self.class_num,          # 计算出来的类别数
                                     m=self.hparams.m,                  # 角度 margin 超参数
                                     h=self.hparams.h,                  # 可能是 head 的附加参数
                                     t_alpha=self.hparams.t_alpha,      # 角度缩放参数
                                     s=self.hparams.s,                  # 特征向量缩放因子
                                     )

        # 5. 定义损失函数: 交叉熵损失函数 CrossEntropyLoss()，用于多类别分类任务。
