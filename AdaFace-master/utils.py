...
"""
    这个 get_num_class 函数的作用是根据 hparams.train_data_path 的不同数据集路径来确定类别数（number of classes）。
    它首先检查 hparams.custom_num_class 是否被手动设置（不等于 -1），如果是，则直接返回该值；否则，它会基于 train_data_path 匹配数据集，并返回对应的类别数。
"""
def get_num_class(hparams):
    # getting number of subjects in the dataset     获取数据集中的类别数量
    # （1）如果 custom_num_class 被手动设置，则直接返回.    hparams.custom_num_class 是用户手动指定的类别数。
    if hparams.custom_num_class != -1:                 # 如果 custom_num_class 不等于 -1，说明用户已经设置了类别数，直接返回，不再计算。
        return hparams.custom_num_class

    # （2）基于 train_data_path 确定类别数.   函数通过 if-elif 结构，检查 hparams.train_data_path 中的关键字，匹配数据集，并设置 class_num。
    if 'faces_emore' in hparams.train_data_path.lower():
        # MS1MV2
        class_num = 70722 if hparams.train_data_subset else 85742           # 如果 train_data_subset=True，类别数为 70,722。否则（完整数据集），类别数为 85,742。
    elif 'ms1m-retinaface-t1' in hparams.train_data_path.lower():
        # MS1MV3
        assert not hparams.train_data_subset            # 不支持子集训练（assert not hparams.train_data_subset）。
        class_num = 93431                               # 类别数固定为 93,431。
